...
class WebappFormatter:
    ...
    def load_mega_analysis(self, match_id: str) -> dict:
        """Load the mega analysis JSON"""
        outputs_dir = Path("../outputs") / match_id
        mega_path = outputs_dir / "mega_analysis.json"
        
        if not mega_path.exists():
            raise Exception(f"mega_analysis.json not found: {mega_path}")
        
        with open(mega_path, 'r') as f:
            return json.load(f)

    def format_events(self, mega_analysis: dict) -> list:
        """Convert mega analysis to web events array"""
        
        prompt = f"""
Convert this football match analysis into a JSON array of events for a webapp timeline.

ANALYSIS DATA:
{json.dumps(mega_analysis, indent=2)}

OUTPUT FORMAT - Return a JSON array where each event has:
{{
  "type": "goal" | "shot" | "foul" | "corner" | "substitution" | "yellow_card" | "red_card",
...
RULES:
1. Extract ALL goals and shots from the analysis
2. Convert timestamps like "32:48" to seconds (32*60 + 48 = 1968)
3. Map teams: Black/White Team -> "red", Blue Team -> "blue"
4. Keep descriptions concise and clear
5. Sort by timestamp ascending
6. Only include events with clear timestamps
...
"""
    ...
    def format_tactical(self, mega_analysis: dict) -> dict:
        """Convert mega analysis to tactical insights"""
        
        prompt = f"""
Convert this football match analysis into tactical insights for a webapp.
...
RULES:
1. Map Black/White Team to red_team, Blue Team to blue_team
2. Extract insights from the tactical analysis section
3. Keep arrays to 2-4 items each
4. Make descriptions coach-friendly and actionable
5. If specific players aren't mentioned, use generic descriptions
