...
Input: data/2-by-country/clubs_by_location.csv (or high_value_prospects.csv)
...
class ContactFinder:
    ...
    def search_club_website(self, club_name, country):
        """Search for club website and extract contacts"""
        try:
            # Enhanced search query with country context
    ...
    def find_club_contacts(self, club_name, country, activity_score):
        ...
        self.logger.info(f"🔍 [{activity_score}] Searching: {club_name} ({country})")
        
        # Search for contacts
        contact_info = self.search_club_website(club_name, country)
    ...
    def process_clubs(self, input_file='data/2-by-country/clubs_by_location.csv', max_clubs=None, min_activity=500):
        ...
        try:
            ...
            if 'Activity_Score' in df.columns:
                df = df[df['Activity_Score'] >= min_activity]  # Focus on high activity clubs
            ...
            self.logger.info(f"📊 Processing {len(df)} clubs (min activity: {min_activity})...")
...
def main():
    ...
    print("=" * 40)
    print("This will find phone numbers and emails for high-value clubs")
    ...
    # Ask user for targeting parameters
    try:
        print("🎯 TARGETING OPTIONS:")
        print("1. High-value only (500+ recordings)")
        print("2. Medium-value (200+ recordings)")  
        print("3. All clubs (50+ recordings)")
        
        choice = input("Choose targeting (1-3): ").strip()
        ...
        max_clubs = input(f"Max clubs to process (Enter for all {min_activity}+ activity): ").strip()
    ...
    # Process clubs
    output_file = finder.process_clubs(input_file, max_clubs, min_activity)
