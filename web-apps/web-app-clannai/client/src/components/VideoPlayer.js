...
const VideoPlayer = ({ session, onClose }) => {
  ...
  // Initialize time range when video is loaded
  useEffect(() => {
  ...
  // Mock events data - in real app this would come from session.ai_analysis
  const mockEvents = [
    { type: 'goal', timestamp: 45, player: 'Player 1', description: 'Great shot into top corner', team: 'red' },
    { type: 'shot', timestamp: 120, player: 'Player 2', description: 'Shot saved by goalkeeper', team: 'black' },
    { type: 'foul', timestamp: 180, player: 'Player 3', description: 'Foul in midfield', team: 'red' },
    { type: 'yellow_card', timestamp: 240, player: 'Player 4', description: 'Yellow card for rough tackle', team: 'black' },
    { type: 'corner', timestamp: 300, player: 'Player 5', description: 'Corner kick awarded', team: 'red' }
  ]
  ...
  const allEvents = mockEvents // In real app: session?.ai_analysis || []
  ...
  }, [currentTime, currentEventIndex, teamScores.red, teamScores.black, allEvents])
  ...
  // Chat functionality (mock for now)
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      const aiMessage = {
        role: 'assistant',
        content: `That's a great question! Based on the analysis of this match, I can see some interesting patterns. The team showed good attacking movement in the first half with several successful passes leading to scoring opportunities.`,
        timestamp: new Date().toISOString()
      }
  ...
  // Get video URL - prioritize analysis video, fallback to footage URL
  const videoUrl = session.analysis_video1_url || session.footage_url

  if (!videoUrl) {
    return (
      <div className="h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">No Video Available</h2>
          <p className="text-gray-400 mb-4">This session doesn't have video content yet.</p>
          <button
            onClick={onClose}
            className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition-colors"
          >
        ...
        <div className="absolute top-0 left-0 h-full w-full md:w-80 bg-black/90 backdrop-blur-sm border-r border-gray-700 flex flex-col z-30">
          <div className="sticky top-0 bg-black/90 backdrop-blur-sm border-b border-gray-700 p-4 z-10">
            ...
            <p className="text-xs text-gray-400">
              Ask questions about tactics, player performance, or get coaching advice based on this match.
            </p>
          </div>
      ...
      <div className={`absolute top-4 z-50 flex items-center space-x-2 md:space-x-4 transition-all duration-300 ${
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-xl px-6 py-2.5 border border-white/20 shadow-lg">
          <div className="flex items-center space-x-4 text-white" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
            <span className="font-semibold text-sm">{session.team?.name || 'Team'}</span>
            ...
            <div className="w-px h-4 bg-white/30"></div>
            <span className="font-medium text-sm text-white/90">Match Analysis</span>
            <div className="w-px h-4 bg-white/30"></div>
            ...
            <div className="relative px-4 pb-3">
              <div className="relative">
                <input
                  ...
                  style={{
                    background: `linear-gradient(to right, #16a34a 0%, #16a34a ${(currentTime / (duration || 1)) * 100}%, rgba(255,255,255,0.2) ${(currentTime / (duration || 1)) * 100}%, rgba(255,255,255,0.2) 100%)`
                  }}
                />
                {/* Progress indicator thumb */}
                <div 
                  className="absolute top-1/2 w-4 h-4 bg-white rounded-full shadow-lg transform -translate-y-1/2 pointer-events-none border-2 border-green-600 transition-all duration-150"
