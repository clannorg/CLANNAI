...
class EventsSynthesis:
    ...
    def load_raw_events(self) -> list:
        ...
        # Find all event text files (prefer high-quality files)
        text_files = list(output_dir.glob("events_analysis_hq_*.txt"))
        if not text_files:
            # Fallback to regular files if no high-quality files found
        ...
        for text_file in text_files:
            ...
            clip_timestamp = filename.replace('events_analysis_hq_', '').replace('events_analysis_', '')
